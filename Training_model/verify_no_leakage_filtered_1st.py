"""
Verify no data leakage in experiment E (train clean -> test filtered_1st_stage).

Checks (replays the same split construction as train_cross_filtered_1st.py):
  1. Every filtered_1st_stage cache file exists and the track_id / genre
     labels line up with labels.csv.
  2. Clean and filtered_1st_stage caches share identical y and track_id
     arrays (same window grid -> safe to apply the clean-trained model
     to the 1st-stage-filtered windows of the same track at test time).
  3. For each fold:
       a. train and test track sets are disjoint  (no track leakage),
       b. their union covers all tracks,
       c. the track IDs behind train windows (CLEAN cache) and test
          windows (FILTERED_1ST cache) are disjoint,
  ...
  5. The fold structure is byte-identical to A/B/C/D -- so E is
...
def main() -> int:
    ...
    required = ["X_clean.npy", "y_clean.npy", "t_clean.npy",
                "X_filtered_1st_stage.npy",
                "y_filtered_1st_stage.npy",
                "t_filtered_1st_stage.npy"]
    missing  = [p for p in required if not (CACHE_DIR / p).is_file()]
    if missing:
        fail(f"missing caches: {missing} -- run train_cross_filtered_1st.py first")

    yc = np.load(CACHE_DIR / "y_clean.npy"); tc = np.load(CACHE_DIR / "t_clean.npy")
    yf = np.load(CACHE_DIR / "y_filtered_1st_stage.npy")
    tf = np.load(CACHE_DIR / "t_filtered_1st_stage.npy")
    Xc_shape = np.load(CACHE_DIR / "X_clean.npy",               mmap_mode="r").shape
    Xf_shape = np.load(CACHE_DIR / "X_filtered_1st_stage.npy",  mmap_mode="r").shape

    print("[1] filtered_1st_stage cached labels self-consistent with labels.csv")
    for src, y, t in [("clean", yc, tc), ("filtered_1st_stage", yf, tf)]:
    ...
    print("[2] clean and filtered_1st_stage caches share window grid")
    if Xc_shape != Xf_shape:
        fail(f"feature-matrix shape mismatch: clean={Xc_shape} filtered_1st={Xf_shape}")
    if not np.array_equal(yc, yf): fail("y_clean != y_filtered_1st_stage")
    if not np.array_equal(tc, tf): fail("t_clean != t_filtered_1st_stage")
    ok(f"y and track_id arrays identical between clean and filtered_1st_stage "
    ...
    print("[3] per-fold train(clean)/test(filtered_1st_stage) track disjointness")
    ...
    for k, (tr_idx, te_idx) in enumerate(splits):
        ...
        # cross-source disjointness on track id
        ...
        ok(f"fold {k+1}: train(clean)={len(tr_tracks)} tracks / {len(tr_win_clean)} windows, "
           f"test(filtered_1st)={len(te_tracks)} tracks / {len(te_win_filt)} windows -- disjoint track ids")
    ...
    print("[5] same fold structure as A/B/C/D (so E is directly comparable)")
    ...
    for k, ((a_tr, a_te), (b_tr, b_te)) in enumerate(zip(splits, splits_f)):
        if not (np.array_equal(a_tr, b_tr) and np.array_equal(a_te, b_te)):
            fail(f"fold {k+1}: clean/filtered_1st splits differ -- E is NOT directly comparable")
    ok("filtered_1st_stage uses byte-identical fold assignments to clean/noisy/filtered_AI")

    print("\nALL CHECKS PASSED -- no track leakage and matched folds for E.")
    return 0
