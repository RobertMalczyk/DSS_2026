"""
Verify no data leakage in experiment F (train clean -> test filtered_2nd_stage).

Checks (replays the same split construction as train_cross_filtered_2nd.py):
  1. filtered_2nd_stage cache is self-consistent with labels.csv.
  2. Clean and filtered_2nd_stage caches share identical y and track_id
...
def main() -> int:
    ...
    required = ["X_clean.npy", "y_clean.npy", "t_clean.npy",
                "X_filtered_2nd_stage.npy",
                "y_filtered_2nd_stage.npy",
                "t_filtered_2nd_stage.npy"]
    missing  = [p for p in required if not (CACHE_DIR / p).is_file()]
    if missing:
        fail(f"missing caches: {missing} -- run train_cross_filtered_2nd.py first")

    yc = np.load(CACHE_DIR / "y_clean.npy"); tc = np.load(CACHE_DIR / "t_clean.npy")
    yf = np.load(CACHE_DIR / "y_filtered_2nd_stage.npy")
    tf = np.load(CACHE_DIR / "t_filtered_2nd_stage.npy")
    Xc_shape = np.load(CACHE_DIR / "X_clean.npy",              mmap_mode="r").shape
    Xf_shape = np.load(CACHE_DIR / "X_filtered_2nd_stage.npy", mmap_mode="r").shape

    print("[1] filtered_2nd_stage cached labels self-consistent with labels.csv")
    for src, y, t in [("clean", yc, tc), ("filtered_2nd_stage", yf, tf)]:
    ...
    print("[2] clean and filtered_2nd_stage caches share window grid")
    if Xc_shape != Xf_shape:
        fail(f"feature-matrix shape mismatch: clean={Xc_shape} filtered_2nd={Xf_shape}")
    if not np.array_equal(yc, yf): fail("y_clean != y_filtered_2nd_stage")
    if not np.array_equal(tc, tf): fail("t_clean != t_filtered_2nd_stage")
    ok(f"y and track_id arrays identical between clean and filtered_2nd_stage "
    ...
    print("[3] per-fold train(clean)/test(filtered_2nd_stage) track disjointness")
    ...
    for k, (tr_idx, te_idx) in enumerate(splits):
        ...
        ok(f"fold {k+1}: train(clean)={len(tr_tracks)} tracks / {len(tr_win_clean)} windows, "
           f"test(filtered_2nd)={len(te_tracks)} tracks / {len(te_win_filt)} windows -- disjoint track ids")
    ...
    print("[5] same fold structure as A/B/C/D/E (so F is directly comparable)")
    ...
    for k, ((a_tr, a_te), (b_tr, b_te)) in enumerate(zip(splits, splits_f)):
        if not (np.array_equal(a_tr, b_tr) and np.array_equal(a_te, b_te)):
            fail(f"fold {k+1}: clean/filtered_2nd splits differ -- F is NOT directly comparable")
    ok("filtered_2nd_stage uses byte-identical fold assignments to clean/noisy/filtered_AI/filtered_1st")

    print("\nALL CHECKS PASSED -- no track leakage and matched folds for F.")
    return 0
