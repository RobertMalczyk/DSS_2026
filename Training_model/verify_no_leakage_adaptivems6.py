"""
Verify no data leakage in experiment G (train clean -> test filtered_adaptivems_6th_stage).

Checks (replays the same split construction as train_cross_adaptivems6.py):
  1. filtered_adaptivems_6th_stage cache is self-consistent with labels.csv.
  2. Clean and adaptivems_6th_stage caches share identical y and track_id
     arrays (same window grid).
  3. For each fold:
...
def main() -> int:
    ...
    required = ["X_clean.npy", "y_clean.npy", "t_clean.npy",
                "X_filtered_adaptivems_6th_stage.npy",
                "y_filtered_adaptivems_6th_stage.npy",
                "t_filtered_adaptivems_6th_stage.npy"]
    missing  = [p for p in required if not (CACHE_DIR / p).is_file()]
    if missing:
        fail(f"missing caches: {missing} -- run train_cross_adaptivems6.py first")

    yc = np.load(CACHE_DIR / "y_clean.npy"); tc = np.load(CACHE_DIR / "t_clean.npy")
    yg = np.load(CACHE_DIR / "y_filtered_adaptivems_6th_stage.npy")
    tg = np.load(CACHE_DIR / "t_filtered_adaptivems_6th_stage.npy")
    Xc_shape = np.load(CACHE_DIR / "X_clean.npy",                         mmap_mode="r").shape
    Xg_shape = np.load(CACHE_DIR / "X_filtered_adaptivems_6th_stage.npy", mmap_mode="r").shape

    print("[1] filtered_adaptivems_6th_stage cached labels self-consistent with labels.csv")
    for src, y, t in [("clean", yc, tc), ("adaptivems_6th_stage", yg, tg)]:
    ...
    print("[2] clean and adaptivems_6th_stage caches share window grid")
    if Xc_shape != Xg_shape:
        fail(f"feature-matrix shape mismatch: clean={Xc_shape} adaptivems={Xg_shape}")
    if not np.array_equal(yc, yg): fail("y_clean != y_filtered_adaptivems_6th_stage")
    if not np.array_equal(tc, tg): fail("t_clean != t_filtered_adaptivems_6th_stage")
    ok(f"y and track_id arrays identical between clean and adaptivems_6th_stage "
    ...
    print("[3] per-fold train(clean)/test(adaptivems_6th_stage) track disjointness")
    ...
    for k, (tr_idx, te_idx) in enumerate(splits):
        ...
        test_win_tracks  = set(int(x) for x in tg[te_win_filt])
        ...
        ok(f"fold {k+1}: train(clean)={len(tr_tracks)} tracks / {len(tr_win_clean)} windows, "
           f"test(adaptivems_6th)={len(te_tracks)} tracks / {len(te_win_filt)} windows -- disjoint track ids")
    ...
    print("[5] same fold structure as A/B/C/D/E/F (so G is directly comparable)")
    unique_tracks_g = np.unique(tg)
    track_genre_g   = np.array([y_lab[int(t)] for t in unique_tracks_g], dtype=np.int64)
    sgkf_g = StratifiedGroupKFold(n_splits=N_SPLITS, shuffle=True, random_state=SEED)
    splits_g = list(sgkf_g.split(
        X=np.zeros((len(unique_tracks_g), 1)),
        y=track_genre_g,
        groups=unique_tracks_g,
    ))
    for k, ((a_tr, a_te), (b_tr, b_te)) in enumerate(zip(splits, splits_g)):
        if not (np.array_equal(a_tr, b_tr) and np.array_equal(a_te, b_te)):
            fail(f"fold {k+1}: clean/adaptivems_6th splits differ -- G is NOT directly comparable")
    ok("adaptivems_6th_stage uses byte-identical fold assignments to the baseline")

    print("\nALL CHECKS PASSED -- no track leakage and matched folds for G.")
    return 0
